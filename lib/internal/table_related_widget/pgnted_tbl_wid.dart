...
import 'package:glamgear/internal/table_data_source_models/tbl_dta_src.dart';

class PaginatedTableWidget extends StatefulWidget {
  const PaginatedTableWidget({super.key});
...
class _PaginatedTableWidgetState extends State<PaginatedTableWidget> {
  final TableDataSource _dataSource = TableDataSource()
    ..setData(_episodes, 0, true);
  ...
  void _sort(int columnIndex, bool ascending) {
    setState(() {
      ...
      _dataSource.setData(_episodes, _columnIndex, _columnAscending);
    });
  }
  ...
  Widget build(BuildContext context) {
    return PaginatedDataTable(
      ...
      columns: <DataColumn>[
        DataColumn(
          label: RetainTextScaleWrapper(child: const Text('Episode')),
          onSort: _sort,
        ),
        DataColumn(
          label: RetainTextScaleWrapper(child: const Text('Title')),
          onSort: _sort,
        ),
        DataColumn(
          label: RetainTextScaleWrapper(child: const Text('Director')),
          onSort: _sort,
        ),
        DataColumn(
          label: RetainTextScaleWrapper(child: const Text('Writer(s)')),
          onSort: _sort,
        ),
        DataColumn(
          label: RetainTextScaleWrapper(child: const Text('Air Date')),
          onSort: _sort,
        ),
      ],
      source: _dataSource,
...
final List<List<Comparable<Object>>> _episodes = <List<Comparable<Object>>>[
  <Comparable<Object>>[
    1,
    1,
    1,
